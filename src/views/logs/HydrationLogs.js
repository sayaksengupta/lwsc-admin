...
const HydrationLogs = () => {
  ...
  const fetchLogs = async (page = 1) => {
    ...
    try {
      ...
      const response = await logsApi.getHydrationLogs(params);
      ...
      console.error(error);
      message.error('Failed to fetch hydration logs');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = () => {
    ...
    const url = logsApi.getExportUrl('hydration', params);
  ...
  const columns = [
    ...
    { title: 'Amount (oz)', dataIndex: 'amountOz', key: 'amountOz', render: (amt) => <b>{amt} oz</b> },
    { title: 'Type', dataIndex: 'type', key: 'type', render: (type) => <Tag color="cyan">{type?.toUpperCase()}</Tag> },
  ...
  return (
    <Card bordered={false} className="shadow-sm">
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Title level={4} style={{ margin: 0 }}>Hydration Logs</Title>
        <Space>
